...
trait LoadParameters
{
    ...
    public function load(array $parameters)
    {
        ...
        $creating = preg_match('/CreateUserProfile$/', $called_class);
        $updating = preg_match('/UpdateUserProfile$/', $called_class);
        ...
        if ($updating && key_exists('role_id', $parameters) && $this->user->is_myself) {
            //  todo?
        }
